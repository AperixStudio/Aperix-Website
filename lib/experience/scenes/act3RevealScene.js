...
function createScreenPlane(video, guideColor, options = {}) {
  const texture = createVideoTexture(video);
  const cornerRadiusRatio = options.cornerRadiusRatio ?? 0;

  let material;

  if (cornerRadiusRatio > 0) {
    material = createRoundedScreenMaterial({
      map: texture,
  ...
  mesh.userData.roundedScreen = cornerRadiusRatio > 0;
  return mesh;
}

function updateAct3RoundedScreenUniforms(mesh, spec, liveConfig) {
  if (!mesh.userData.roundedScreen) {
    return;
  }

  applyRoundedScreenUniforms(mesh.material, {
    aspect: spec.height / Math.max(spec.width, 0.0001),
    cornerRadius: liveConfig.iphoneScreenCornerRadius ?? 0.105,
  });
}
...
function applyScreenMaterialMode(mesh, showGuides) {
  ...
  if (isRoundedScreenMaterial(material)) {
    applyRoundedScreenUniforms(material, {
      guideMode: showGuides,
      opacity: showGuides ? 0.35 : 1,
    });
    material.visible = showGuides || Boolean(texture?.image?.readyState >= 2);
    return;
  }
...
export function createAct3RevealScene({
  ...
  const camera = new THREE.PerspectiveCamera(config.cameraFov ?? 42, 1, 0.1, 100);
  ...
  function attachScreenPlanes() {
    if (iphoneRoot && !iphoneScreenPlane) {
      iphoneScreenPlane = createScreenPlane(iphoneVideo, 0x22c55e, {
        cornerRadiusRatio: resolveConfig(getLiveConfig()).iphoneScreenCornerRadius ?? 0.105,
      });
  ...
  function updateScreenPlanes() {
    ...
    if (iphoneScreenPlane && iphoneRoot) {
      const iphoneSpec = computeAutoScreenSpec(iphoneRoot, "iphone", liveConfig);
      applyScreenSpec(iphoneScreenPlane, iphoneSpec);
      if (iphoneScreenPlane.userData.roundedScreen) {
        updateAct3RoundedScreenUniforms(iphoneScreenPlane, iphoneSpec, liveConfig);
      }
  ...
  function resize(width, height) {
    ...
    camera.fov = resolveConfig(getLiveConfig()).cameraFov ?? 42;
  ...
  function dispose() {
    ...
    iphoneScreenPlane?.geometry?.dispose();
    monitorScreenPlane?.geometry?.dispose();
    iphoneScreenPlane?.material?.dispose();
    monitorScreenPlane?.material?.dispose();
