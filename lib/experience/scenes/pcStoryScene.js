...
import { signalSceneReady, glitchTime, INTRO_STALL_FRACTION } from "@/lib/storyBus";
import { onIntroDone, introHasPlayed } from "@/components/animations/IntroScreen";
...
function computeMonitorFaceFrameFromLockedLocal(screenMesh, box, lock, target) {
  const center = box.getCenter(target.center);

  screenMesh.updateWorldMatrix(true, true);
  const meshQuat = screenMesh.getWorldQuaternion(target.quaternion);

  target.normal.copy(lock.normal).applyQuaternion(meshQuat).normalize();
  target.up.copy(lock.up).applyQuaternion(meshQuat).normalize();
  target.right.copy(lock.right).applyQuaternion(meshQuat).normalize();
  ...
  // Use the plane dimensions captured at lock time. Recomputing them from the
  // world-axis-aligned bounding box makes the plane squish/stretch as the model
  // rotates (e.g. the Act 2 slide), because an AABB's projected extents change
  // with orientation even though the real monitor face never changes size.
  target.planeWidth = lock.planeWidth;
  target.planeHeight = lock.planeHeight;
...
function lockMonitorFaceToMesh(screenMesh, camera, box, frame, storage, scratchQuat, scratchInvQuat) {
  ...
  storage.planeWidth = frame.planeWidth;
  storage.planeHeight = frame.planeHeight;
  storage.locked = true;
}
...
function createIntroLabelTexture(text, fill) {
  ...
  ctx.font = "600 72px Hubot Sans Variable, system-ui, sans-serif";
...
/**
 * Draws the CRT boot screen that mirrors the DOM intro onto a canvas.
 * Uses glitchTime() so the jitter/RGB phase is frame-synchronised with
 * the IntroScreen overlay sitting on top.
 */
function drawBootScreenCanvas(canvas, ctx) {
  const W = canvas.width;
  const H = canvas.height;

  ctx.clearRect(0, 0, W, H);

  // Light-gray base (matches intro background #F3F4F6, visible right-hand sliver)
  ctx.fillStyle = "#F3F4F6";
  ctx.fillRect(0, 0, W, H);

  // Near-black sweep covering INTRO_STALL_FRACTION of the width
  const gt = glitchTime();
  const isGlitching = gt >= 0;
  const stutterOffset = isGlitching
    ? (Math.floor(gt / 0.133) % 3 === 1 ? -0.006 : Math.floor(gt / 0.133) % 3 === 2 ? 0.008 : 0)
    : 0;
  const sweepW = W * Math.max(0, Math.min(1, INTRO_STALL_FRACTION + stutterOffset));
  ctx.fillStyle = "#171717";
  ctx.fillRect(0, 0, sweepW, H);
  ...
  // Jitter offset (mirrors ixJitter keyframes)
  let jx = 0;
  let jy = 0;
  if (isGlitching) {
    const step = Math.floor(gt / 0.0886) % 7;
    const offsets = [[0,0],[-7,2],[5,-1],[-3,1],[8,2],[-5,-2],[2,1]];
    jx = offsets[step][0] * (W / 1440);
    jy = offsets[step][1] * (H / 900);
  }

  // Center of sweep area
  const cx = sweepW * 0.5 + jx;
  const cy = H * 0.5 + jy;

  // RGB split ghost layers (mirrors ixRgb filter)
  if (isGlitching) {
    const rgbStep = Math.floor(gt / 0.105) % 4;
    if (rgbStep > 0) {
      const shifts = [[3,-3],[-4,4],[2,-2]];
      const colors = ["rgba(255,80,80,0.45)","rgba(80,80,255,0.38)","rgba(255,80,80,0.5)"];
      const shift = shifts[Math.min(rgbStep - 1, 2)];
      ctx.save();
      ctx.globalAlpha = 0.6;
      ctx.fillStyle = colors[Math.min(rgbStep - 1, 2)];
      ctx.font = `bold ${Math.round(H * 0.055)}px monospace`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("APERIX", cx + (shift[0] * W / 1440), cy - H * 0.09);
      ctx.restore();
    }
  }
  ...
  ctx.font = `600 ${Math.round(H * 0.055)}px Hubot Sans Variable, system-ui, sans-serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.shadowColor = "rgba(14,165,233,0.55)";
  ctx.shadowBlur = W * 0.022;
  ctx.fillText("APERIX", cx, cy - H * 0.09);
  ctx.shadowBlur = 0;

  // Progress bar
  const barW = W * 0.28;
  const barH = Math.max(6, H * 0.028);
  const barX = cx - barW / 2;
  const barY = cy - barH / 2 + H * 0.02;
  ctx.strokeStyle = "rgba(255,255,255,0.4)";
  ctx.lineWidth = 1.5;
  ctx.strokeRect(barX, barY, barW, barH);

  // Segmented fill (matches the repeating-linear-gradient on the DOM bar)
  const segW = barW * INTRO_STALL_FRACTION - 2;
  const segSize = barW * 0.038;
  ctx.save();
  ctx.beginPath();
  ctx.rect(barX + 1, barY + 1, segW, barH - 2);
  ctx.clip();
  for (let x = barX + 1; x < barX + 1 + segW; x += segSize * 1.4) {
    ctx.fillStyle = "rgba(255,255,255,0.92)";
    ctx.fillRect(x, barY + 1, segSize, barH - 2);
  }
  ctx.restore();

  // Status text — stutter 89/90/91 during glitch phase
  let pct = 0;
  if (isGlitching) {
    const roll = Math.sin(gt * 137);
    pct = roll < -0.64 ? 89 : roll > 0.72 ? 91 : 90;
  } else {
    pct = Math.round(INTRO_STALL_FRACTION * 100);
  }
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  ctx.font = `${Math.round(H * 0.032)}px monospace`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(`LOADING SYSTEM... ${pct}%`, cx, barY + barH + H * 0.045);

  // Slice tears — draw colored rectangles instead of self-copy to avoid GPU stalls
  if (isGlitching) {
    const beat = (gt % 0.9) / 0.9;
    const slices = [
      { phase: 0.12, y: H * 0.12 },
      { phase: 0.44, y: H * 0.44 },
      { phase: 0.74, y: H * 0.74 },
    ...
    for (const s of slices) {
      if (Math.abs(beat - s.phase) < 0.03) {
        const offset = (Math.sin(gt * 73) * 0.5 + 0.5) * 28 - 14;
        ctx.save();
        ctx.globalAlpha = 0.55;
        ctx.fillStyle = "#FFFFFF";
        ctx.fillRect(offset, s.y, W, H * 0.035);
        ctx.restore();
      }
    }
  }
 ...
 * @param {() => number} [options.getModelProgress] Act 1 model + screen rig progress; defaults to camera progress.
 * @param {() => number | null} [options.getAct2SlideProgress] Act 2 slide (0–1), null outside Act 2.
 ...
 * @param {() => import("@/lib/dev/heroTheatre/types").HeroTheatreRig | null} [options.getTheatreRig] Dev Theatre.js direct rig.
...
export function createPcStoryScene({
  ...
  getScrollProgress,
  getModelProgress = getScrollProgress,
  getAct2SlideProgress = () => null,
  ...
  let bootScreenMesh = null;
  let bootScreenCanvas = null;
  let bootScreenCtx = null;
  let bootScreenTexture = null;
  let bootScreenOpacity = 1;
  let bootScreenFadeStart = null;
  ...
  function updateScreenPlaneTransform(config, progress, scaleOverride) {
    ...
    const applyToPlane = (plane) => {
      ...
      const scaleHeight =
        scaleOverride?.height ??
        lerp(config.screenPlaneScaleHeightStart ?? 1, config.screenPlaneScaleHeightEnd ?? 1, progress);

      plane.scale.set(frame.planeWidth * scaleWidth, frame.planeHeight * scaleHeight, 1);
    };
  ...
  function setupBootScreen() {
    ...
    if (bootScreenMesh) {
      scene.remove(bootScreenMesh);
      bootScreenMesh.geometry.dispose();
      bootScreenMesh.material.dispose();
      bootScreenTexture?.dispose();
    }
    bootScreenCanvas = document.createElement("canvas");
    bootScreenCanvas.width = 1024;
    bootScreenCanvas.height = 576;
    bootScreenCtx = bootScreenCanvas.getContext("2d");
    if (!bootScreenCtx) return;

    bootScreenTexture = new THREE.CanvasTexture(bootScreenCanvas);
    bootScreenTexture.colorSpace = THREE.SRGBColorSpace;

    bootScreenMesh = new THREE.Mesh(
      new THREE.PlaneGeometry(1, 1),
      new THREE.MeshBasicMaterial({
        map: bootScreenTexture,
        transparent: true,
        depthWrite: false,
        depthTest: false,
        toneMapped: false,
    ...
    bootScreenMesh.renderOrder = 14;
    scene.add(bootScreenMesh);

    onIntroDone(() => {
      bootScreenFadeStart = performance.now();
    });
  }

  function updateBootScreen(frame) {
    if (!bootScreenMesh || !bootScreenCtx || !bootScreenCanvas || !frame) {
      return;
    }

    // Fade out after intro exits (matches EXIT_MS = 250ms)
    if (bootScreenFadeStart !== null) {
      const elapsed = performance.now() - bootScreenFadeStart;
      bootScreenOpacity = Math.max(0, 1 - elapsed / 250);
      bootScreenMesh.material.opacity = bootScreenOpacity;
      if (bootScreenOpacity <= 0) {
        scene.remove(bootScreenMesh);
        bootScreenMesh.geometry.dispose();
        bootScreenMesh.material.dispose();
        bootScreenTexture.dispose();
        bootScreenMesh = null;
        bootScreenCanvas = null;
        bootScreenCtx = null;
        bootScreenTexture = null;
        bootScreenFadeStart = null;
        return;
      }
    }

    // Position flat against the monitor face
    bootScreenMesh.position.copy(frame.center);
    bootScreenMesh.position.addScaledVector(frame.normal, 0.002);
    bootScreenMesh.quaternion.copy(frame.quaternion);
    bootScreenMesh.scale.set(frame.planeWidth, frame.planeHeight, 1);

    // Redraw canvas each tick while visible
    drawBootScreenCanvas(bootScreenCanvas, bootScreenCtx);
    bootScreenTexture.needsUpdate = true;
  }
  ...
  function getAct1ModelPose(config, progress) {
    return {
      x: lerp(config.modelOffsetStartX, config.modelOffsetEndX, progress),
      y: lerp(config.modelOffsetStartY, config.modelOffsetEndY, progress),
      z: lerp(config.modelOffsetStartZ, config.modelOffsetEndZ, progress),
      rotX: lerp(0, config.modelRotationX ?? 0, progress),
      rotY: lerp(0, config.modelRotationY ?? 0, progress),
      rotZ: lerp(0, config.modelRotationZ ?? 0, progress),
    };
  }
  ...
  function applyLiveConfig(config) {
    ...
    const cameraProgress = getScrollProgress();
    const modelProgress = getModelProgress();
    const act2Slide = getAct2SlideProgress();
    const theatreRig = getTheatreRig();
    const rigProgress = act2Slide != null ? ACT2_CAMERA_HOLD : modelProgress;
    ...
    if (theatreRig) {
      ...
      modelRoot.rotation.set(
        theatreRig.model.rotX ?? 0,
        theatreRig.model.rotY,
        theatreRig.model.rotZ ?? 0,
      );

      const theatreCameraProgress = theatreRig.cameraProgress ?? cameraProgress;
      const startCam = getCameraStartPosition(config);
      const endCam = getCameraEndPosition(config);
      camera.position.lerpVectors(startCam, endCam, theatreCameraProgress);

      const pivotY = originCenter.y * config.cameraOrbitPivotYRatio;
      camera.lookAt(0, pivotY, 0);

      const theatreRigProgress =
        theatreRig.act === 2 ? ACT2_CAMERA_HOLD : theatreCameraProgress;

      updateScreenPlaneTransform(config, theatreRigProgress, {
        width: theatreRig.screenScale.width,
        height: theatreRig.screenScale.height,
      });
      updateIntroLabelTransform(config, theatreCameraProgress);
      updateBootScreen(getMonitorFrame());
      return;
    }

    if (act2Slide != null) {
      // Act 2 model track is fully independent of Act 1 — its own start/end keys.
      // Seam fallback only used if a config predates the act2*Start keys.
      const seam = getAct1ModelPose(config, ACT2_CAMERA_HOLD);

      const startX = config.act2ModelOffsetStartX ?? seam.x;
      const startY = config.act2ModelOffsetStartY ?? seam.y;
      const startZ = config.act2ModelOffsetStartZ ?? seam.z;
      const startRotY = config.act2ModelRotationStartY ?? seam.rotY;

      const endX = config.act2ModelOffsetEndX ?? startX;
      const endY = config.act2ModelOffsetEndY ?? startY;
      const endZ = config.act2ModelOffsetEndZ ?? startZ;
      const endRotY = config.act2ModelRotationEndY ?? config.act2ModelRotationY ?? startRotY;

      modelRoot.position.set(
        lerp(startX, endX, act2Slide),
        lerp(startY, endY, act2Slide),
        lerp(startZ, endZ, act2Slide),
      );

      modelRoot.rotation.set(0, lerp(startRotY, endRotY, act2Slide), 0);
    } else {
      const pose = getAct1ModelPose(config, modelProgress);

      modelRoot.position.set(pose.x, pose.y, pose.z);
      modelRoot.rotation.set(pose.rotX, pose.rotY, pose.rotZ);
    }

    const startCam = getCameraStartPosition(config);
    const endCam = getCameraEndPosition(config);
    camera.position.lerpVectors(startCam, endCam, cameraProgress);

    const pivotY = originCenter.y * config.cameraOrbitPivotYRatio;
    camera.lookAt(0, pivotY, 0);

    // Act 2 gets its own constant floating-screen size, independent of Act 1's
    // start→end scale lerp. Falls back to the held Act 1 value if keys are absent.
    const screenScaleOverride =
      act2Slide != null
        ? {
            width:
              config.act2ScreenPlaneScaleWidth ??
              lerp(
                config.screenPlaneScaleWidthStart ?? 1,
                config.screenPlaneScaleWidthEnd ?? 1,
                ACT2_CAMERA_HOLD,
              ),
            height:
              config.act2ScreenPlaneScaleHeight ??
              lerp(
                config.screenPlaneScaleHeightStart ?? 1,
                config.screenPlaneScaleHeightEnd ?? 1,
                ACT2_CAMERA_HOLD,
              ),
          }
        : null;

    updateScreenPlaneTransform(config, rigProgress, screenScaleOverride);
    updateIntroLabelTransform(config, modelProgress);
    updateBootScreen(getMonitorFrame());
  }
