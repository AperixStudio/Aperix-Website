...
const MODEL_PATHS = ["/retro_pc_comp.glb", "/retro_electronics_retro_pc.glb"];
...
export function createPcStoryScene({
  ...
  function updateIntroLabelTransform(config, progress) {
    ...
    introLabelMesh.visible = getShowIntroLabel() && getScreenEvolutionProgress() < 0.04;
  }
  ...
  function onModelLoaded(gltf, pathIndex, modelPath) {
    ...
    if (!root) {
      console.error(`PC model has no scene root (${modelPath})`, gltf);
