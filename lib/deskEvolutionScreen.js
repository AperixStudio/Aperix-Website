...
import { ACT2_WIREFRAME_END, ACT2_WIREFRAME_START } from "@/lib/homeStoryTimeline";
...
export function drawDeskScreen(ctx, progress) {
  const blueprintWeight = 1 - smoothstep(ACT2_WIREFRAME_START - 0.10, ACT2_WIREFRAME_START, progress);
  const wireframeWeight =
    smoothstep(ACT2_WIREFRAME_START - 0.02, ACT2_WIREFRAME_START + 0.08, progress) *
    (1 - smoothstep(ACT2_WIREFRAME_END - 0.02, ACT2_WIREFRAME_END + 0.08, progress));
  const liveWeight = smoothstep(ACT2_WIREFRAME_END + 0.04, ACT2_WIREFRAME_END + 0.24, progress);
